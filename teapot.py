...
logging.basicConfig(filename='/var/lib/teapot/webdav/teapot.log', level=logging.getLevelName(LOGLEVEL))
...
async def _return_or_create_storm_instance(sub):
    ...
    if (exists("/var/lib/teapot/user-"+local_user+"/server.pid")):
        with open("/var/lib/teapot/user-"+local_user+"/server.pid",'r') as pid_file:
            pid=pid_file.read()
        with open("/var/lib/teapot/user-"+local_user+"/server.port","r") as port_file:
            port=port_file.read().rstrip()
        logger.info(f"StoRM-WebDAV instance for {local_user} is running on port {port}.")  # noqa: E501
    else:
        port=8085
        subprocess.run(["sudo", "/usr/share/teapot/webdav-automation.sh", local_user, str(port)])  # noqa: E501
        with open("/var/lib/teapot/user-"+local_user+"/server.pid",'r') as pid_file:
